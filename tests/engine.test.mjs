...
test("user message ingest is durable and recalled into the system prompt", async () => {
  ...
  try {
    ...
    handlers.onMessageEnd({ message: u("Avery lives in Austin.") }, ctx);
...
test("toolCall-only assistant messages are ingested (the call is recallable)", async () => {
  ...
  try {
    ...
    handlers.onMessageEnd({
      message: aCall("exec", { command: "tar -czf backup-vermilion.tgz /srv/data", timeout: 10 }),
    }, ctx);
...
test("tool results are ingested in full", async () => {
  ...
  try {
    const { handlers } = build(dir);
    const ctx = fakeCtx({ sessionId: "S" });
    handlers.onMessageEnd({ message: tr("backup written: 4183 files, checksum qz88x") }, ctx);
...
test("default (session) scope does NOT recall across sessions", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir); // default memoryScope: session
    handlers.onMessageEnd({ message: u("Avery lives in Austin.") }, fakeCtx({ sessionId: "A" }));
...
test("ISOLATION: session scope prevents cross-session recall; same session recalls", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { memoryScope: "session" });
    handlers.onMessageEnd({ message: u("Avery lives in Austin.") }, fakeCtx({ sessionId: "A" }));
...
test("agent scope recalls across sessions of the same project", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { memoryScope: "agent" });
    handlers.onMessageEnd({ message: u("Avery lives in Austin.") }, fakeCtx({ sessionId: "A", cwd: "/tmp/proj" }));
...
test("ISOLATION: agent scope isolates distinct projects", async () => {
  ...
  try {
    const { handlers } = build(dir, { memoryScope: "agent" });
    handlers.onMessageEnd({ message: u("Avery lives in Austin.") }, fakeCtx({ sessionId: "A", cwd: "/tmp/project-a" }));
...
test("no stale recall after an update in the same session (no cross-turn cache)", async () => {
  ...
  try {
    ...
    handlers.onMessageEnd({ message: u("Avery moved to Boston.") }, ctx);
...
test("context handler injects a display:false custom message with fresh recall", async () => {
  ...
  try {
    ...
    const ctx = fakeCtx({ sessionId: "S" });
    handlers.onMessageEnd({ message: u("Avery lives in Austin.") }, ctx);
    const messages = [u("Where does Avery live?"), at("I will check.")];
...
test("context handler does not re-inject lines already in the system-prompt block", async () => {
  ...
  try {
    ...
    const out = handlers.onContext({ messages: [u("Where does Avery live?")] }, ctx);
...
test("context handler drains gate-staged recall under the same scope key", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers, bus } = build(dir);
    const ctx = fakeCtx({ sessionId: "S" });
    handlers.onMessageEnd({ message: u("The rollback runbook is in the wiki.") }, ctx);
    // stage directly under the session's scope key (what the gate does)
    bus.stage("s-S", "- the rollback runbook is in the wiki");
    const out = handlers.onContext({ messages: [u("Where is the rollback runbook?")] }, ctx);
    assert.ok(out?.messages, "staged recall was drained into a context message");
    assert.equal(bus.take("s-S"), "", "drain clears the staged block");
  } finally { cleanup(); }
});
...
test("message_end skips non-conversation roles and empty content", async () => {
  ...
  try {
    ...
    assert.doesNotThrow(() => {
      ...
      handlers.onMessageEnd({ message: u("   ") }, ctx);
    });
...
test("compaction: hybrid cut is exchange-aligned, zero-LLM summary, entry ids", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { compactionMode: "hybrid", protectTail: 4 });
    const ctx = fakeCtx({ sessionId: "S" });
    // Six exchanges; ingest a fact in the first so compaction has real memory behind it.
    handlers.onMessageEnd({ message: u("The deploy freeze ends on the 14th.") }, ctx);
    const entries = [
    ...
    const out = handlers.onSessionBeforeCompact(
      { preparation: prep(entries, entries[0].id), branchEntries: entries, reason: "manual", willRetry: false, signal: new AbortController().signal },
      ctx,
    );
    ...
    assert.equal(out.compaction.details.mode, "hybrid");
  } finally { cleanup(); }
});
...
test("compaction: full mode cuts at the last user message", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { compactionMode: "full" });
    const ctx = fakeCtx({ sessionId: "S" });
    const entries = [
      msgEntry(u("q1")), msgEntry(at("a1")),
      msgEntry(u("q2")), msgEntry(at("a2")),
    ...
    const out = handlers.onSessionBeforeCompact(
      { preparation: prep(entries, entries[0].id), branchEntries: entries, reason: "threshold", willRetry: false, signal: new AbortController().signal },
      ctx,
    );
    assert.equal(out?.compaction?.firstKeptEntryId, entries[10].id, "current exchange kept");
    assert.equal(out.compaction.details.mode, "full");
  } finally { cleanup(); }
});
...
test("compaction: nothing before the protected window -> pi default (undefined)", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { compactionMode: "hybrid", protectTail: 8 });
    const ctx = fakeCtx({ sessionId: "S" });
    const entries = [msgEntry(u("hello")), msgEntry(at("hi")), msgEntry(u("ok"))];
    const out = handlers.onSessionBeforeCompact(
      { preparation: prep(entries, entries[0].id), branchEntries: entries, reason: "manual", willRetry: false, signal: new AbortController().signal },
      ctx,
    );
...
test("compaction: respects the previous compaction's kept boundary", async () => {
  ...
  try {
    ...
    const eOlder = msgEntry(u("older"));
    const eOlder2 = msgEntry(at("older2"));
    const entries = [
      eOld, eOld2, eOlder, eOlder2,
      compactionEntry("c1", eOlder.id),
      msgEntry(u("kept-q")), msgEntry(at("kept-a")),
      msgEntry(u("q")), msgEntry(at("a")),
      msgEntry(u("q2")), msgEntry(at("a2")),
      msgEntry(u("q3")), msgEntry(at("a3")),
    ];
    const out = handlers.onSessionBeforeCompact(
      { preparation: prep(entries, entries[10].id), branchEntries: entries, reason: "manual", willRetry: false, signal: new AbortController().signal },
      ctx,
    );
...
test("compaction: falls back to pi's own cut when no user boundary exists in range", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { compactionMode: "hybrid", protectTail: 2 });
    const ctx = fakeCtx({ sessionId: "S" });
    ...
    const eOld = msgEntry(u("old"));
    const eOldA = msgEntry(at("old-a"));
    const entries = [
      eOld,
      eOldA,
      compactionEntry("c1", eOldA.id),
      msgEntry(at("a1")),
      msgEntry(at("a2")),
      msgEntry(at("a3")),
    ];
    const out = handlers.onSessionBeforeCompact(
      { preparation: prep(entries, entries[4].id), branchEntries: entries, reason: "manual", willRetry: false, signal: new AbortController().signal },
      ctx,
    );
...
test("post-compaction recall: archived fact comes back through the memory block", async () => {
  ...
  try {
    ...
    const ctx = fakeCtx({ sessionId: "S" });
    handlers.onMessageEnd({ message: u("The deploy freeze ends on the 14th.") }, ctx);
    handlers.onMessageEnd({ message: at("Noted.") }, ctx);
    handlers.onMessageEnd({ message: u("Filler one about the weather and the on-call rotation.") }, ctx);
    handlers.onMessageEnd({ message: at("Ok.") }, ctx);
    handlers.onMessageEnd({ message: u("Filler two about the quarterly review agenda items.") }, ctx);
    handlers.onMessageEnd({ message: at("Sure.") }, ctx);
    handlers.onMessageEnd({ message: u("Filler three about the accessibility audit contrast fixes.") }, ctx);
    handlers.onMessageEnd({ message: at("Got it.") }, ctx);
    ...
    const kept = [u("When does the deploy freeze end?")];
...
test("interruptGate=false: gate events are safe no-ops", async () => {
  const { dir, cleanup } = tempDir();
  try {
    const { handlers } = build(dir, { interruptGate: false });
    const ctx = fakeCtx({ sessionId: "S" });
    const msg = at("answer");
    assert.doesNotThrow(() => {
...
test("session_shutdown flushes engines (idempotent)", async () => {
  ...
  try {
    const { handlers } = build(dir);
    const ctx = fakeCtx({ sessionId: "S" });
    handlers.onMessageEnd({ message: u("A fact for shutdown.") }, ctx);
    assert.doesNotThrow(() => {
